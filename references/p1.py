...
a='''劉禹錫 王維 孟浩然 李白 白居易 杜牧 高適 元稹 李賀 韓愈 王昌齡 韋應物 岑參 賈島 溫庭筠 李商隱 劉長卿 許渾 韋莊 陸龜蒙 皮日休 張九齡 宋之問 王勃 駱賓王 張說 陳子昂 綦毋潛 王建'''
for x in a.split():
    x=x.strip()
#    print x
    ...
    cmd = 'ebook-convert %s.recipe %s.mobi' %(x,x)
    print cmd
    cmd = 'ebook-convert %s.recipe %s.epub' %(x,x)
    print cmd
    #break # debug
